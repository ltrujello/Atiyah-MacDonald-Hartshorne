...
\begin{solution}
    ...
    % \lfy{
    % (i) $\Rightarrow$ (ii). Since $Z(\fa) = \emptyset$, the zero set of $\fa$ in affine space \begin{luke}Did you mean projective space?} is either $\emptyset$ or $\{\mathbf{0}\}.$ In the first case, we certainly have $\sqrt{\fa} = S$. In the second case, we have $\sqrt{\fa} = \{p \in S: p(0) = 0\} = S_+$.
